...
`git clone https://github.com/ByteLegendQuest/java-correct-final`

A project directory named `java-correct-final` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-correct-final` directory which is created just now.
